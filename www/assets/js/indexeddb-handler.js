...
var indexedDbHandler = {
  ...
  initializeDb: function() {
    return new Promise(async function(resolve, reject) {
      ...
      openRequest.onupgradeneeded = async function(e) {
        ...
        await indexedDbHandler.upgradeDatabase(e.oldVersion, upgradeTransaction);
          ...
          store.openCursor().onsuccess = function(event) {
            ...
            if (cursor) {
              ...
              if (value.portion == "Quick Add" && data != undefined && data.quickAddId !== undefined) {
                ...
                item.unit = value.portion.replace(app.Utils.nonLettersRegExp(), "");
            ...
            if (cursor) {
              ...
              if (portion !== undefined) {
                if (typeof portion == "string") {
                  ...
                  item.unit = portion.replace(app.Utils.nonLettersRegExp(), "");
                } else {
                ...
                for (let idx in value.foods) {
                  ...
                  if (f.portion !== undefined) {
                    if (typeof f.portion == "string") {
                      ...
                      item.unit = f.portion.replace(app.Utils.nonLettersRegExp(), "");
                    } else {
          ...
          store.openCursor().onsuccess = function(event) {
            ...
            if (cursor) {
              ...
              if (typeof value.portion == "string")
                recipe.unit = value.portion.replace(app.Utils.nonLettersRegExp(), "");
  ...
  put: function(data, storeName, key) {
    ...
    request.onerror = function(err) {
      indexedDbHandler.errorHandler(err);
    };
  ...
  get: function(storeName, index, key) {
    return new Promise(function(resolve, reject) {
      ...
      request.onerror = function(e) {
        indexedDbHandler.errorHandler(e);
        reject(e);
      };
    });
  },
  ...
  getIndexSorted: function(storeName, indexName, direction, range) {
    direction = direction || 'next';

    return new Promise(function(resolve, reject) {
      let index = DB.transaction(storeName).objectStore(storeName).index(indexName);
      let keyRange = range ? IDBKeyRange.bound(range.lower, range.upper) : null;
      let results = [];

      let cursorRequest = index.openCursor(keyRange, direction === "prev" ? "prev" : "next");
            ...
            data[x].forEach(async (d) => {
              ...
              request.onsuccess = async (e) => {
                ...
                if (storeCount >= storeNames.length) {
                  await indexedDbHandler.upgradeDatabase(version, t, {
